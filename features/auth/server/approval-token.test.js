...
test("an approval token is tied to the cashier, the discount and five minutes", () => {
  ...
  expect(readApproval(secret, token, { cashierId: "u-cashier", discountPct: 10, now: now + APPROVAL_TTL_MS - 1 })).toEqual({ supervisorId: "u-manager", expiresAt: now + APPROVAL_TTL_MS })
