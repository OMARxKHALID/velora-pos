...
const rowSchema = z
  .object({
    ...
    size: z.string().trim().regex(/^\d{2}(\.5)?$/, { error: "size must be an EU size like 42" }),
...
export const parseCatalogImport = (text) => {
  ...
  lines.forEach((cells, index) => {
    ...
    const parsed = rowSchema.safeParse({ ...record, barcode: record.barcode ?? "", stock: record.receive || 0 })
