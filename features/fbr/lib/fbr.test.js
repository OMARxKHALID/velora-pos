...
import { applyExchange, applyOpenShift, applyPurchase, applyRefundDecision, applyRefundRequest, applySale, emptyLedger, reportFbr } from "@/features/ledger/lib/rules"
...
describe("fbr payloads", () => {
  ...
  test("the sale gets a fiscal number online and waits for sync offline", () => {
    ...
    const { fbr } = reportFbr(state, sale.fbr, at)
    expect(fbr.status).toBe("reported")
    expect(fbr.invoiceNumber).toMatch(/^110014\d{16}$/)
  })
