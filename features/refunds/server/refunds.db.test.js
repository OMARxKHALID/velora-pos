...
import { resetDemoData } from "@/features/demo/server/team"
...
describe.skipIf(!hasTestDatabase)("returns, settings and what each role can read", () => {
  ...
  test("each role reads only what it should", async () => {
    ...
    expect(cashier.sales.every(({ cashierId }) => cashierId === "u-cashier")).toBe(true)
    expect(cashier.sales.flatMap(({ items }) => items).every((item) => !("unitCost" in item))).toBe(true)
    expect(cashier.variants.every((item) => !("cost" in item))).toBe(true)
    expect(cashier.movements).toEqual([])
    expect(cashier.purchases).toEqual([])
    const supervisor = await ledgerSnapshot(context.db, { user: { id: "u-manager", role: "manager", shopId: SHOP } })
    expect(supervisor.movements.length).toBeGreaterThan(0)
    expect(supervisor.heldCarts).toEqual([])
    expect(supervisor.sales.length).toBeGreaterThan(cashier.sales.length - 1)
  ...
  test("demo reset puts the shop back to fresh sample data", async () => {
    await resetDemoData({ db: context.db })
