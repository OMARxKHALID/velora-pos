...
export const ShopDialog = ({ shop = null, onClose }) => {
  const saveShop = useDemoStore(({ saveShop }) => saveShop)
  const deleteShop = useDemoStore(({ deleteShop }) => deleteShop)
...
export const CounterDialog = ({ shop, register = null, onClose }) => {
  const saveRegister = useDemoStore(({ saveRegister }) => saveRegister)
  const [draft, setDraft] = useState({
    name: register?.name ?? "",
