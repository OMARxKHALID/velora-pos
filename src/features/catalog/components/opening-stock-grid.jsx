...
export const OpeningStockGrid = ({ colors, sizes, value, onChange, unit = "pairs" }) => {
  ...
  if (!colors.length || !sizes.length) return <p className="text-sm text-muted-foreground">Add colours and sizes above, then enter how many you have.</p>

  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-end gap-2">
        <Input value={fill} onChange={(event) => setFill(event.target.value)} inputMode="numeric" placeholder="e.g. 5" className="w-24 tabular-nums" aria-label="Quantity for every size" />
        <Button type="button" size="sm" variant="outline" onClick={handleFill}>
          Set all sizes
        </Button>
        <span className="ml-auto text-sm text-muted-foreground">
          Total <span className="font-semibold text-foreground tabular-nums">{total}</span> {unit}
        </span>
      </div>
      {colors.map((color) => (
            ...
            {sizes.map((size) => {
              ...
              return (
                <label key={key} className="flex flex-col items-center gap-1 border px-1 pt-1 pb-1.5 focus-within:border-primary">
                  <span className="text-2xs text-muted-foreground tabular-nums">{sizeLabel(size)}</span>
                  <input
                    value={value[key] ?? ""}
                    onChange={(event) => onChange({ ...value, [key]: event.target.value.replace(/\D/g, "").slice(0, 3) })}
                    inputMode="numeric"
                    placeholder="0"
                    aria-label={`${color} ${sizeLabel(size)} in stock`}
