...
export const purchaseSchema = z.object({
  ...
  lines: z
    .array(z.object({ variantId: z.string(), quantity: z.coerce.number().int({ error: "Whole numbers only" }).min(1, { error: "At least 1" }).max(500) }))
...
export const adjustmentSchema = (onHand) =>
  z
    .object({
      ...
      quantity: z.coerce.number().int({ error: "Whole numbers only" }).min(1, { error: "At least 1" }).max(500),
